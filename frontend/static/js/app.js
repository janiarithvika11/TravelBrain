...
(function () {
  ...
  var sidebarNewTripBtn = $("sidebarNewTripBtn");
  var historyList = $("historyList");
  var sidebarShield = $("sidebarShield");
  ...
  function renderHistoryList(threads) {
    ...
    if (threads.length === 0) {
      ...
      empty.textContent = "No recent trips.";
    ...
    threads.forEach(function (thread) {
      ...
      titleElem.textContent = thread.title || "Untitled Trip";
      ...
      delBtn.innerHTML = '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>';

      delBtn.addEventListener("click", function (e) {
        e.stopPropagation();
        if (confirm("Are you sure you want to delete this trip planner session?")) {
  ...
  function deleteThread(idToDelete) {
    fetch("/api/history/" + idToDelete, { method: "DELETE" })
      .then(function (r) {
        if (r.ok) {
  ...
  function loadThread(idToLoad) {
    if (busy) { toast("Wait for the current planner to finish."); return; }

    // Set active item visually immediately
    var items = historyList.querySelectorAll(".history-item");
    items.forEach(function (el) {
      if (el.getAttribute("data-id") === idToLoad) {
    ...
    // Hide mobile drawer overlay if open
    layoutContainer.classList.remove("sidebar-open");

    fetch("/api/history/" + idToLoad)
      .then(function (r) {
      ...
      .then(function (data) {
        if (data.success) {
          ...
          toast("Failed to load conversation details.");
        }
      })
      .catch(function (err) {
  ...
  function startNewTrip() {
    ...
    // Clear active UI classes in sidebar list
    var items = historyList.querySelectorAll(".history-item");
    items.forEach(function (el) {
      el.classList.remove("active");
    });
  ...
  sidebarShield.addEventListener("click", function () {
    layoutContainer.classList.remove("sidebar-open");
  });

  sidebarNewTripBtn.addEventListener("click", startNewTrip);
